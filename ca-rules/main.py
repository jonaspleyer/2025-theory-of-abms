...
def plot_rule(rule, start: np.ndarray, folder: Path):
    ...
    binary_name, decimal_name = generate_rule_name(rule)
    if np.sum(total[1:]) > 0 and np.any(total[1] != total[0]):
        folder.mkdir(parents=True, exist_ok=True)
        img = Image.fromarray(255 * total.astype(np.uint8))
        img.save(folder / f"rule-{decimal_name}.png", dpi=(300, 300))
    else:
        pass
        # print(binary_name, decimal_name)
...
if __name__ == "__main__":
    ...
    for rule in tqdm(ca_rule_generator(), total=32):
        plot_rule(rule, start, folder=Path("single1"))
    ...
    for rule in tqdm(ca_rule_generator(), total=32):
        plot_rule(rule, start, folder=Path("double1-spaced"))
    ...
    for rule in tqdm(ca_rule_generator(), total=32):
        plot_rule(rule, start, folder=Path("double1-close"))
    ...
    for rule in tqdm(ca_rule_generator(), total=32):
        plot_rule(rule, start, folder=Path("alternating"))
