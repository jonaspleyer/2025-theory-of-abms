...
%---------------------------------------------------------------------------------------------------
\newpage
\subsection{Coarse-graining of a Bacterial Pool Model}
...
We assume that each cell is represented by a position $\textbf{x}_c\in\R^d$ and interacts in a
point-wise manner with the external environment.
This apparoach is general enough to cover a wide range of cellular systems while also providing the
...
\begin{alignat}{5}
    ...
    \partial^2_t\textbf{x}_c &= &\sum\limits_{s\neq c}& \textbf{F}(\textbf{x}_c, \textbf{x}_s) - \lambda \partial_t\textbf{x}_c
...
\begin{table}[H]
    ...
    \begin{tabular}{lclll}
        ...
        Division                & $\gamma(c)$                               & $\CS$             & $\rightarrow$ & $\CS\times\CS$\\
        \bottomrule
    \end{tabular}
...
\begin{equation}
    ...
    \text{E}\left(\frac{V_c}{V_\mathscr{D}}\right)\text{E}(\bar{\textbf{h}}_{i,c}({\bar{\textbf{w}}}))
    + \bar{\textbf{h}}_e(\bar{\textbf{w}})
    = \bar{\textbf{h}}(\bar{\textbf{w}})
\end{equation}
We have used the expectation value $\text{E}(X_c)=\sum_c X_c$ to factorize the first term.
In the final step, we used a very concise notation, showcasing that our equations reduce to an \ac{ode}.
...
\begin{align}
    \textbf{u}_0 &= \text{E}(\bar{\textbf{v}}_c) - q\bar{\textbf{w}}\\
    \textbf{u}_1 &= \text{E}(\bar{\textbf{v}}_c) + \bar{\textbf{w}}\\
    \textbf{z}_i &= \text{E}(\bar{\textbf{h}}_e(\bar{\textbf{w}}))\\
    \textbf{z}_e &= \bar{\textbf{h}}_e(\bar{\textbf{w}})
\end{align}
where $q=\text{E}(V_c/V_\mathscr{D})$, we are left with the system
\begin{alignat}{3}
    \partial_t\textbf{u}_0 &= \textbf{z}_i &-q& \textbf{z}_e -(1+q)\textbf{A}\textbf{u}_0 \\
    \partial_t\textbf{u}_1 &= \textbf{z}_i &+& \textbf{z}_e
\end{alignat}
where the two equations are only decoupled if $\textbf{z}_{i/e}$ does not depend on $\textbf{u}_{0/1}$.
