...
\begin{alignat}{5}
    \partial_t{V_c} &= &&&&h_g(\textbf{v}_c, V_c)
...
Equation~\eqref{eq:volumetric-growth} defines the term $h_g(\textbf{v}_c, V_c)$ which describes the rate of change of the volume of a single cell.
...
\begin{table}[H]
    ...
    \begin{tabular}{lclll}
        ...
        \midrule
        Growth Model            & $h_g(\textbf{v}_c,V_c)$                   & $\R^n\times\R$    & $\rightarrow$ & $\R$\\
...
We consider the case with no cellular growth $h_g=0$, and aim to exclude any spatial dependence by integrating over the domain $\mathscr{D}$.
...
The first term can be understood as an expectation value of a product of two distributions over living cells $c\in\CS$ which we will use to rewrite equation~\eqref{eq:pool-model-case1-coarse-grained-result}.
...
\end{alignat}
We have used the expectation value $\text{E}(X_c)=\sum_c X_c$ to factorize the first term and $q=\text{E}(V_c/V_\mathscr{D})$ with $\bar{\textbf{h}}_i=\text{E}(\bar{\textbf{h}}_{i,c})$ to simplify notation.
