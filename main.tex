...
\begin{alignat}{7}
    ...
    &= &\sum\limits_c& \frac{V_c}{V_\mathscr{D}}\bar{\textbf{h}}_{i,c}(\bar{\textbf{w}}) &+& \bar{\textbf{h}}_e(\bar{\textbf{w}})
...
The first term can be understood as an expectation value $\text{E}(X_c)=\sum_cX_c$ of a product of two distributions over living cells $c\in\CS$ which we will use to rewrite equation~\eqref{eq:pool-model-case1-coarse-grained-result}.
...
\begin{equation}
    \partial_t\bar{\textbf{w}} =
    \text{E}\left(\frac{V_c}{V_\mathscr{D}}\right)\text{E}(\bar{\textbf{h}}_{i,c}({\bar{\textbf{w}}}))
    + \text{Cov}\left(\frac{V_c}{V_\mathscr{D}},\bar{\textbf{h}}_{i,c}({\bar{\textbf{w}}})\right)
    + \bar{\textbf{h}}_e(\bar{\textbf{w}})
\end{equation}
...
\begin{alignat}{5}
    ...
    &\text{E}\left(\frac{V_c}{V_\mathscr{D}}\right)&\text{E}(\bar{\textbf{h}}_{i,c}({\bar{\textbf{w}}}))
    &+& \bar{\textbf{h}}_e(\bar{\textbf{w}})\\
    &= &q& \bar{\textbf{h}}_i(\textbf{w}) &+& \bar{\textbf{h}}_e(\bar{\textbf{w}})
\end{alignat}
We have used the expectation value to factorize the first term and $q=\text{E}(V_c/V_\mathscr{D})$ with $\bar{\textbf{h}}_i=\text{E}(\bar{\textbf{h}}_{i,c})$ to simplify notation.
Using this concise notation, the final step shows that our equations reduce to an \ac{ode}.
