\documentclass[referee,pdflatex,sn-mathphys-ay]{sn-jnl}% Math and Physical Sciences Author Year Reference Style
...
\begin{table}[H]
    ...
    \begin{tabular}{cll}
        ...
        \toprule
        (C) & Mechanics & Spherical, Rod, Hexagonal, Puzlle-shaped\\
...
\begin{itemize}
    \item Link to previous section
    \item Treat \ac{abm} as black-box model $\Omega(x, t)$
    \item Estimate parameters by comparing output to data
\end{itemize}
...
\end{align}
and the substitutions $\bar{\textbf{h}}_i=\text{E}(\bar{\textbf{h}}_{i,c}(\bar{\textbf{w}}))$ and $q=\text{E}(V_c/V_\mathscr{D})$, we obtain the system
\begin{alignat}{3}
...
\begin{alignat}{5}
    \partial_tV &= &&&\sum\limits_c &h_{g,c}(\textbf{w}, V_c)\\
